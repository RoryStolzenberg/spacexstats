            ...
            @elseif ($mission->upperStage->upperstage_status == 'Earth Orbit')
                ...
                @if ($mission->orbitalElements->count() > 0)
                    <p class="orbital-details">
                        {{ round($mission->orbitalElements->first()->perigee) }}km x {{ round($mission->orbitalElements->first()->apogee) }}km <small>inclined</small> {{ $mission->orbitalElements->first()->inclination }}&deg;</p>
                    <p>({{ round($mission->orbitalElements->first()->period, 1) }} minutes)</p>
                    <p>As of {{ $mission->orbitalElements->first()->epoch->toFormattedDateString() }}</p>

                    <span>{{ $mission->orbitalElements->count() }} TLE's</span>
                @endif
            @endif
        </div>
    </div>
